...
class FaceMatcher:
    ...
    def find_match(self, input_vector, threshold=0.6):
        if not self.known_users:
            print("ไม่มีข้อมูล User ในระบบเลย! (Known Users is empty)")
            return {"found": False}
        ...
        for user in self.known_users:
            ...
            if vec1.shape != vec2.shape:
                print(
                    f"Dimension mismatch: vec1 shape {vec1.shape}, vec2 shape {vec2.shape}. "
                    f"Skipping user {user.get('id', 'unknown')}."
                )
                continue
